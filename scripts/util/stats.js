/*
 * https://github.com/Tigenzero/bitburner-scripts/blob/master/get_stats.js
 */
/**
 * @param {NS} ns
 **/
  ...
  while (i < servers.length) {
    ...
    for (var j in s) {
      ...
      if (servers.indexOf(con) < 0) {
        ...
        if (
          all ||
          (ns.hasRootAccess(con) && parseInt(ns.getServerMaxMoney(con)) > 0)
        ) {
          result.push(con);
        }
      }
...
function get_server_data(ns, server) {
  ...
  var skill_required = ns.getServerRequiredHackingLevel(server);
  var ram = ns.getServerMaxRam(server);
  return (
    `${pad_str(server, 17)}` +
    ` money:${pad_str(parseInt(moneyAvailable), 12)}/${pad_str(
      parseInt(moneyMax),
      12
    )}(${pad_str((moneyAvailable / moneyMax).toFixed(2), 4)})` +
    ` security:${pad_str(securityLvl.toFixed(2), 6)}(${pad_str(
      securityMin,
      2
    )})` +
