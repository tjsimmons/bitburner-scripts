// https://gist.github.com/quonic/2350d9acad0fd280f3c65a458f93ef44
...
*/
  ...
  ns.print(
    `Waiting to purchase next upgrade (${ns.hacknet.numNodes()} nodes)...`
  );
...
function displayDashBoard(ns) {
  ...
  let maxNodes = ns.hacknet.numNodes() < numNodes ? numNodes : Infinity;
  ...
  ns.print(
    `Total Production: ${
      nodes.length === 0
        ...
        : ns.nFormat(
            nodes
              .map((v, i) => ns.hacknet.getNodeStats(i).production)
              .reduce((a, b) => a + b),
            MoneyFormat
          )
    } /s`
  ...
  ns.print(
    `Total Produced: ${
      nodes.length === 0
        ...
        : ns.nFormat(
            nodes
              .map((v, i) => ns.hacknet.getNodeStats(i).totalProduction)
              .reduce((a, b) => a + b),
            MoneyFormat
          )
    }`
  ...
  ns.print(
    table(
      ...
      nodes.map((v, i) =>
        ns.nFormat(ns.hacknet.getNodeStats(i).totalProduction, MoneyFormat)
      ),
      nodes.map((v, i) =>
        ns.nFormat(ns.hacknet.getNodeStats(i).timeOnline, TimeFormat)
      ),
      nodes.map(
        (v, i) =>
          `${ns.nFormat(ns.hacknet.getNodeStats(i).production, MoneyFormat)} /s`
      ),
